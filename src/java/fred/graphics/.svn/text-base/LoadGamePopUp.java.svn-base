...
import static fred.graphics.ImagePaths.LOADGAME_MENU;
import static fred.graphics.ImagePaths.LOADGAME_TEXTBOX;
...
public class LoadGamePopUp extends PopUp {
    ...
    protected static final String WARNING_CANNOT_READ_FILE = "Cannot read save file";
    ...
    public LoadGamePopUp(GameContainer gc, GameUI stateGame, float scalar, boolean inGame) throws SlickException {
        super(gc, LOADGAME_MENU, scalar, inGame);
        this.stateGame = stateGame;

        addComponent(getButtonBack());
        addComponent(getButtonAccept());
        addComponent(getTextFieldName());
        textBoxImg = createImage(LOADGAME_TEXTBOX);
    }
        ...
        if (buttonBack == null) {
            ...
            buttonBack.addListener(new ComponentListener() {
                public void componentActivated(AbstractComponent source) {
                    LoadGamePopUp.this.close(Result.Cancel);
                }
            });
        }
        return buttonBack;
    ...
    private Button getButtonAccept() throws SlickException {
        if (buttonAccept == null) {
            ...
            buttonAccept = createButton(container, activeImage, createImage(GENERAL_ACCEPT_PRESSED, scalar),
                    getX() + getWidth() - 120, getY() + 120);
            buttonAccept.addListener(new ComponentListener() {
                public void componentActivated(AbstractComponent source) {
                    ...
                    if (cleanFilename != null && cleanFilename.equals(filename)) {
                        ...
                        try {
                            saver.load(cleanFilename + ".sav");
                        }
                        catch (IllegalArgumentException ex) {
                            warningText = WARNING_CANNOT_READ_FILE;
                            showWarning = true;
                            return;
                        }
                        ...
                        LoadGamePopUp.this.close(Result.Ok);
                    }
